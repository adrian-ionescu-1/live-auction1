          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-gray-600 mb-2">
                Players Won ({user.wonPlayers.length})
              </p>
              {user.wonPlayers.length > 0 ? (
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {user.wonPlayers.map((playerWon, index) => (
                    <div
                      key={`${playerWon.playerId}-${index}`}
                      className="bg-blue-50 rounded-lg p-2 text-sm"
                    >
                      <p className="font-semibold text-gray-800">{playerWon.playerName}</p>
                      <p className="text-xs text-gray-600">
                        Bought for: <span className="font-bold text-green-600">${playerWon.amount.toLocaleString()}</span>
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-4 pt-3 border-t-2 border-gray-100">
              <div className="flex justify-between items-center">
                ...
                <p className="text-lg font-bold text-red-600">
                  ${user.wonPlayers.reduce((sum, p) => sum + p.amount, 0).toLocaleString()}
                </p>
              </div>
            </div>
          </div>
